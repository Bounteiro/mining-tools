...
local utils = (function()
    ...
    function self.addChat(a)
        ...
        -- Наша внутренняя строка в UTF-8 — переводим обратно в cp1251 перед отправкой в чат SAMP.
        sampAddChatMessage(u8:decode('{ffa500}' .. thisScript().name .. '{ffffff}: ' .. a), -1)
    end
    ...
    function self.debugChat(a)
        ...
        -- Наша внутренняя строка в UTF-8 — переводим обратно в cp1251 перед отправкой в чат SAMP.
        sampAddChatMessage(u8:decode('{ffa500}' .. thisScript().name .. ' DEBUG' .. '{ffffff}: ' .. a), -1)
    end
...
-- FIX: пункты меню банка в телефоне (phoneBankMenuId) раньше выбирались по
-- жёстко зашитому номеру строки (например, "4-я строка — это налоги").
-- Если сервер меняет порядок пунктов меню (или сам dialogId сдвигается),
-- скрипт молча тыкает не туда, и деньги никуда не уходят, хотя внешне
-- выглядит так, будто всё отработало. Вместо жёсткого номера строки теперь
-- ищем нужный пункт ПО ТЕКСТУ (по ключевым словам), а номер строки —
-- это просто запасной вариант на случай, если текст не удалось разобрать.

-- Убирает цветовые теги вида {RRGGBB} и лишние пробелы, приводит к нижнему
-- регистру — чтобы сравнение по ключевым словам не зависело от оформления.
function stripDialogColorTags(line)
    line = (line or ""):gsub("{%x%x%x%x%x%x}", "")
    line = line:gsub("^%s+", ""):gsub("%s+$", "")
    return line
end

-- Разбирает многострочный текст диалога-списка (listbox) на пронумерованные
-- (с нуля, как ожидает sampSendDialogResponse) пункты.
function parseDialogListItems(text)
    local items = {}
    local index = 0
    for line in (text or ""):gmatch("[^\r\n]+") do
        table.insert(items, { index = index, raw = line, clean = stripDialogColorTags(line) })
        index = index + 1
    end
    return items
end

-- Ищет среди распарсенных пунктов меню первый, чей текст содержит любой из
-- шаблонов (обычная подстрока, без учёта регистра). Возвращает найденный
-- индекс и true, либо запасной индекс (fallbackIndex) и false, если ничего
-- не подошло (например, меню ещё не было показано/распознано).
function findMenuItemIndexByText(items, patterns, fallbackIndex)
    if items and #items > 0 then
        for _, item in ipairs(items) do
            local hay = item.clean:lower()
            for _, pattern in ipairs(patterns) do
                if hay:find(pattern:lower(), 1, true) then
...
-- Дожидается появления конкретного dialogId (с таймаутом) и отправляет в
-- него ответ с пунктом, найденным ПО ТЕКСТУ среди patterns. Если меню банка
-- ещё не было замечено онШоуДиалог-обработчиком (или ключевые слова не
...
function sendMenuActionByText(sendResponse, dialogIdToWaitFor, patterns, fallbackIndex, label, maxWaitMs)
    ...
    if not foundByText then
        utils.debugChat(string.format(
            "[BANK] Пункт меню \"%s\" не найден по тексту, использую индекс по умолчанию: %d",
            label, fallbackIndex))
    ...
    sendResponse(dialogIdToWaitFor, 1, resolvedIndex, "")
        ...
        if resp.status_code == 200 or resp.status_code == 304 then
            ...
            if ok and info and info.latest then
                if info.latest ~= script.this.version then
                    ...
                    updateState.changelog     = (info.changelog or "")
                    updateState.showPopup[0]  = false
...
local collectTool = (function()
    ...
    function self.postponeCollect(minutes)
        ...
        imgui.addNotification((string.format("Сбор отложен на %d мин.", minutes)))
    end
...
local helpTool = (function()
    ...
    local function actionBtn(icon, label, id, onClick)
        ...
        if imgui.Button(icon .. "  " .. (label) .. "##help_" .. id, imgui.ImVec2(-1, 28)) then
            onClick()
        end
    ...
    local setupPages = {
        ...
        },
        {
            ...
            render = function()
                ...
                if imgui.SliderInt("##setBal", imcfg.targetHouseBalance, 5000000, 60000000,
                        ("$" .. utils.formatNumber(imcfg.targetHouseBalance[0]))) then
        ...
        },
        {
            ...
            render = function()
                ...
                imgui.Spacing()
                imgui.PushItemWidth(-1)
                if imgui.SliderInt("##setPause", imcfg.pause_duration, 150, 300, "%d мс") then
                    cfg.pause_duration = imcfg.pause_duration[0]; save()
                end
                if imgui.SliderInt("##setCount", imcfg.count_action, 1, 20,
                        (string.format("пауза каждые %d", imcfg.count_action[0]))) then
        ...
        },
        {
            ...
            render = function()
                ...
                imgui.Spacing()
                if imgui.Checkbox("Авто-заливка жидкости при открытии стойки", imcfg.fixCoolantEnabled) then
                    cfg.fixCoolantEnabled = imcfg.fixCoolantEnabled[0]; save()
                end
                imgui.Hint("Доливать охлаждающую жидкость сразу при открытии стойки.")
                if imgui.Checkbox("Авто-включение карт при открытии стойки", imcfg.autoEnableCardsOnOpen) then
    ...
    local refPages = {
        ...
        },
        {
            ...
            render = function()
                ...
                imgui.Spacing()
                imgui.PushItemWidth(-1)
                if imgui.SliderInt("##refPause", imcfg.pause_duration, 150, 300, "%d мс") then
                    cfg.pause_duration = imcfg.pause_duration[0]; save()
                end
        ...
        },
        {
            ...
            render = function()
                ...
                if imgui.SliderInt("##refDur", imcfg.notifyShowDuration, 3, 30,
                        (string.format("показывать %d сек.", imcfg.notifyShowDuration[0]))) then
    ...
    local function pager(pages, pageKey, idPrefix)
        ...
        if imgui.Button(fa.ARROW_LEFT .. "  " .. ("Назад") .. "##" .. idPrefix .. "Prev", imgui.ImVec2(btnW, 30)) and not atStart then
        ...
        if imgui.Button(("Далее") .. "  " .. fa.ARROW_RIGHT .. "##" .. idPrefix .. "Next", imgui.ImVec2(btnW, 30)) and not atEnd then
            data[pageKey] = data[pageKey] + 1
        end
...
function formatTimeLeft(seconds)
    if seconds <= 0 then return "уже пора!" end
...
local function checkAndSwitchProfile(sn, nick)
    ...
    cfg.isReloaded = true
    save()
    utils.addChat("{FFE133}Смена профиля, перезагрузка...")
...
local function updateConnectionState()
    ...
    if nowConnected ~= wasConnected then
        if nowConnected then
            ...
            data.hasFlashminer = nil
            data.initialScanCompleted = false
            data.noHousesNotified = false
...
function main()
    ...
    lua_thread.create(function()
        ...
        if not skipWelcome then
            ...
            utils.addChat("{808080}Профиль: {FFFFFF}" .. tostring(nick) .. "{808080} / {FFFFFF}" .. tostring(sn))
            utils.addChat("{808080}Готово. {ffc0cb}/mnt{ffffff}")
        end
    end)
    ...
    if not cfg.incomeStatsResetV2 and type(cfg.cardSnapshots) == 'table' then
        ...
        save()
        utils.debugChat("[INCOME] Старая статистика дохода по домам сброшена, счётчик начнёт накапливаться заново.")
    end
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text, placeholder)
    -- Диалоговый текст приходит от сервера в cp1251 — конвертируем в UTF-8
    -- один раз здесь, чтобы дальше по коду сравнения с русскими литералами
    -- (которые в этом файле хранятся в UTF-8) работали корректно.
    title = u8(title or "")
    text = u8(text or "")
    button1 = button1 or ""
    button2 = button2 or ""
...
if cfg.afkMode and dialogId ~= data.dFlashminerId then
    ...
    -- FIX: запоминаем содержимое меню банка (phoneBankMenuId) КАЖДЫЙ раз,
    -- когда оно реально показано на экране — чтобы дальше можно было искать
    -- нужный пункт ("оплата налогов", "пополнение баланса") ПО ТЕКСТУ, а не
    -- по жёстко зашитому номеру строки, который сервер может сдвинуть.
    if dialogId == dialogIdTable.phoneBankMenuId then
...
function sampev.onServerMessage(color, text)
    -- Текст чата приходит от сервера в cp1251 — конвертируем в UTF-8 один раз,
    -- чтобы сравнения с русскими литералами ниже (хранятся в UTF-8) совпадали.
    text = u8(text or "")
    if not cfg.active then return end
        ...
        elseif (snapshot.time or 0) > 0 and timeDiffMinutes >= MIN_INTERVAL then
            ...
            if diff > 0 then
                ...
                if isBtcOutlier then
                    utils.debugChat(string.format(
                        "[INCOME] Дом №%d: наблюдение похоже на выброс (%.2f BTC/день при подтверждённых %.2f) - не учтено",
            ...
            if diffAsc > 0 then
                ...
                if isAscOutlier then
                    utils.debugChat(string.format(
                        "[INCOME] Дом №%d: наблюдение похоже на выброс (%.2f ASC/день при подтверждённых %.2f) - не учтено",
                    ...
                    if #snapshot.incomeAscObs >= 3 then
                        ...
                        utils.debugChat(string.format(
                            "[INCOME] Дом №%d: %.2f ASC/день (наблюд: %d, diff: %.3f за %.1f мин)",
...
function resetIncomeRates()
    ...
    save()
    utils.addChat("Данные о доходах сброшены.")
end
        ...
        return taskState.ifNotWorking(function()
            ...
            lua_thread.create(function()
                ...
                if not success then
                    ...
                    print(u8:decode("{F78181}Критическая ошибка: " .. tostring(err)))
                    if sampIsDialogActive() then
                        sampCloseCurrentDialogWithButton(0)
                    end
                end
        ...
        task.run = function(self)
            ...
            createProtectedTask(function(sendResponse)
                ...
                for i, house in ipairs(actualHousesToProcess) do
                    if data.stopAction then break end
                    data.currentCollectHouse = (string.format("Дом №%d (%d/%d)",
    ...
    task.run = function(self)
        createProtectedTask(function(sendResponse)
            ...
            elseif not data.stopAction then
                -- FIX: раньше здесь БЕЗ ПОДТВЕРЖДЕНИЯ от сервера писалось
                -- "Налоги оплачены" и обновлялось lastTaxPayTime — то есть
                -- если сервер не ответил (например, клик попал не в тот
                -- пункт меню из-за сдвига), скрипт врал, что всё оплачено,
                -- и следующая попытка откладывалась на полный интервал.
                -- Теперь честно сообщаем о неудаче и НЕ трогаем
                -- lastTaxPayTime, чтобы таймер повторил попытку раньше.
                utils.addChat(
                    "{F78181}Не удалось подтвердить оплату налогов: сервер не прислал сообщение об оплате за 10 сек. Проверьте вручную.")
                utils.debugChat("[TAX] Оплата не подтверждена сервером (таймаут 10с).")
            end
        end)
    end
        ...
        task.run = function(self, housesToTopUp)
            ...
            createProtectedTask(function(sendResponse)
                ...
                for i, house in ipairs(housesToTopUp) do
                    ...
                    data.currentCollectHouse = (string.format(
...
imgui.OnFrame(
    ...
    function(self)
        ...
        if imgui.Begin("##mntNotify", data.notifyWindow.show, flags) then
            ...
            imgui.SameLine(0, 6)
            imgui.TextColored(imgui.ImVec4(1, 1, 1, _nAlpha), "Mining Tools")
            imgui.Separator()
            imgui.Spacing()
            ...
            if mode == 'reminder' then
                ...
                imgui.TextColored(imgui.ImVec4(1, 1, 1, _nAlpha), (amtText))
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(1, 1, 1, _nAlpha), fa.CIRCLE_EXCLAMATION)
                imgui.SameLine(0, 6)
                imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, _nAlpha), "Рекомендуется собрать криптовалюту.")
            elseif mode == 'countdown' then
                ...
                imgui.SameLine(0, 6)
                local cdText = secsLeft <= 0 and "Собираем..."
                    or (string.format("Автосбор через: %s", formatTimeLeft(secsLeft)))
                ...
                if data.notifyWindow.source == 'smart' then
                    subText = (string.format("Цель: %d BTC+ASC", cfg.smartCollectTarget))
                else
                    subText = (string.format("%d сборов в день", cfg.collectTimesPerDay))
                end
                imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, _nAlpha), subText)
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.45, 0.45, 0.45, _nAlpha * 0.8),
                    isChatOpen and "ПКМ — отложить на 2 минуты" or "T + ПКМ — отложить на 2 минуты")
            elseif mode == 'collecting' then
                imgui.TextColored(imgui.ImVec4(1, 1, 1, _nAlpha), fa.ROTATE)
                imgui.SameLine(0, 6)
                imgui.TextColored(imgui.ImVec4(1, 1, 1, _nAlpha), "Автосбор выполняется...")
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(1, 1, 1, _nAlpha), fa.HOUSE)
                imgui.SameLine(0, 6)
                local houseText = (data.currentCollectHouse ~= "" and data.currentCollectHouse) or "Подготовка..."
                ...
                imgui.ProgressBar(prog, imgui.ImVec2(-1, 14),
                    (string.format("%d / %d домов", data.progressCurrent,
                        data.progressTotal > 0 and data.progressTotal or 0)))
                imgui.PopStyleColor(2)
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.45, 0.45, 0.45, _nAlpha * 0.8),
                    isChatOpen and "ПКМ — отменить сбор" or "T + ПКМ — отменить сбор")
            end
...
imgui.OnFrame(
    ...
    function(self)
        ...
        if imgui.Begin("##settingsWin", data.showSettingsWindow, settingsFlags) then
            ...
            local tabs = { "Общее", "Фермы", "Авто", "Прочее", "Помощь" }
            ...
            do
                ...
                if data.settingsTab == 0 then
                    if imgui.Checkbox("Тихий режим", imcfg.silentMode) then
                    ...
                    elseif UPDATE_CHECK_URL ~= nil then
                        ...
                        if updateState.hasUpdate then
                            ...
                            if imgui.Selectable(fa.ARROW_UP_FROM_BRACKET .. (string.format("  Установить обновление %s", updateState.latestVersion or "")), false) then
                                updateState.showPopup[0]   = true
                                data.showSettingsWindow[0] = false
                            end
                            imgui.PopStyleColor()
                    ...
                    if imgui.Selectable("Просмотр логов", false) then
                    ...
                    if imgui.Selectable("Перезагрузить скрипт", false) then
                ...
                elseif data.settingsTab == 1 then
                    if not cfg.useDialogMode and not data.isRodina then
                        ...
                        if imgui.Checkbox("Только пополнение баланса", imcfg.useSimpleTopUp) then
                        ...
                        if cfg.useSimpleTopUp then
                            ...
                            imgui.Checkbox("Собирать крипту", imgui.new.bool(cfg.fixCollectEnabled))
                            imgui.Checkbox("Включать видеокарты", imgui.new.bool(cfg.fixSwitchEnabled))
                            imgui.Checkbox("Пополнять баланс", imgui.new.bool(cfg.fixTopUpEnabled))
                            imgui.PopStyleColor(4)
                            imgui.Hint("Недоступно: включён режим «Только пополнение баланса».")
                        else
                            if imgui.Checkbox("Собирать крипту", imcfg.fixCollectEnabled) then
                                cfg.fixCollectEnabled = imcfg.fixCollectEnabled[0]; save()
                            end
                            imgui.Hint("Снимать криптовалюту со всех домов.")
                            if imgui.Checkbox("Включать видеокарты", imcfg.fixSwitchEnabled) then
                                cfg.fixSwitchEnabled = imcfg.fixSwitchEnabled[0]; save()
                            end
                            imgui.Hint("Включать выключенные карты.")
                            if imgui.Checkbox("Пополнять баланс", imcfg.fixTopUpEnabled) then
                        ...
                        imgui.PushItemWidth(-1)
                        if imgui.SliderInt("##coolantPercentSettings", imcfg.useCoolantPercent, 1, 100, "%d%%") then
                        ...
                        if imgui.Checkbox("Авто-заливка при открытии стойки", imcfg.fixCoolantEnabled) then
                            cfg.fixCoolantEnabled = imcfg.fixCoolantEnabled[0]; save()
                        end
                        imgui.Hint(
                            "Автоматически заливать жидкость при открытии стойки видеокарт.\nНе работает через Флешку Майнера.")

                        if imgui.Checkbox("Авто-включение карт после заливки", imcfg.autoEnableCards) then
                        ...
                        imgui.Text("Порог баланса (предупреждение):")
                        imgui.PushItemWidth(-1)
                        if imgui.SliderInt("##minBalanceWarning", imcfg.minBalanceWarning, 1000000, 15000000,
                                ("$" .. utils.formatNumber(imcfg.minBalanceWarning[0]))) then
                        ...
                        imgui.Text("Собирать если накопилось не менее:")
                        imgui.PushItemWidth(-1)
                        if imgui.SliderInt("##collectOnlyIfMin", imcfg.collectOnlyIfMin, 0, 180,
                                imcfg.collectOnlyIfMin[0] == 0
                                and "Любое кол-во"
                                or (string.format("от %d BTC", imcfg.collectOnlyIfMin[0]))) then
                        ...
                        if imgui.Selectable("Проверить подвалы", false) then
                            if not data.working then
                                local task = buildTaskTable('scanBasements')
                                runTaskAndReopenDialog(function() task:run(nil) end)
                                data.showSettingsWindow[0] = false
                ...
                elseif data.settingsTab == 2 then
                    if not cfg.useDialogMode and not data.isRodina then
                        ...
                        if imgui.Checkbox("Включить авто-функции", imcfg.cheatModeEnabled) then
                        ...
                        do
                            local subTabs = { "Автосбор", "Финансы", "Уведомления" }
                            ...
                            if data.cheatSubTab == 0 then
                                if not cfg.cheatModeEnabled then
                                    ...
                                    if imgui.Checkbox("Включить автосбор по расписанию", imcfg.autoCollectEnabled) then
                                    ...
                                    if cfg.autoCollectEnabled then
                                        ...
                                        if imgui.SliderInt("##cTimes", imcfg.collectTimesPerDay, 1, 8,
                                                (string.format("%d/день (~%s)", imcfg.collectTimesPerDay[0],
                                        ...
                                        if imgui.Selectable("Сбросить таймер", false) then
                                            cfg.lastCollectTime = os.time()
                                            collectTool.resetPendingDelay()
                                            save()
                                        end
                                    ...
                                    if imgui.Checkbox("Включить умный автосбор", imcfg.smartCollectEnabled) then
                                    ...
                                    if cfg.smartCollectEnabled then
                                        ...
                                        if imgui.SliderInt("##sTgt", imcfg.smartCollectTarget, sMin, sMax,
                                                (string.format("при %d BTC+ASC", imcfg.smartCollectTarget[0]))) then
                                        ...
                                        if sOk then
                                            ...
                                            local accText = string.format(
                                                "{808080}Накоплено: {BEF781}%d BTC+ASC {808080}/ {FFFFFF}%d BTC+ASC",
                                                math.floor(sTotal), cfg.smartCollectTarget)
                                            imgui.TextColoredRGB(accText)

                                            if sA > 0 then
                                                imgui.TextColoredRGB(string.format(
                                                    "{808080}Из них: {D2691E}%d BTC {808080}+ {C0392B}%d ASC",
                                    ...
                                    if cfg.autoCollectEnabled or cfg.smartCollectEnabled then
                                        if imgui.Checkbox("Включать карты после автосбора", imcfg.autoEnableCardsOnCollect) then
                                    ...
                                    if imgui.Checkbox("Добавлять рандомную задержку", imcfg.randomDelayEnabled) then
                                    ...
                                    if cfg.randomDelayEnabled then
                                        ...
                                        if imgui.SliderInt("##rndMin", imcfg.randomDelayMin, 1, imcfg.randomDelayMax[0],
                                                (string.format("от %d мин.", imcfg.randomDelayMin[0]))) then
                                        ...
                                        if imgui.SliderInt("##rndMax", imcfg.randomDelayMax, imcfg.randomDelayMin[0], 180,
                                                (string.format("до %d мин.", imcfg.randomDelayMax[0]))) then
                                    ...
                                    if imgui.Checkbox("Периодически обновлять данные домов", imcfg.autoRefreshEnabled) then
                                    ...
                                    if cfg.autoRefreshEnabled then
                                        ...
                                        if imgui.SliderInt("##refreshInt", imcfg.autoRefreshInterval, 5, 120,
                                                (string.format("каждые %d мин.", imcfg.autoRefreshInterval[0]))) then
                                        ...
                                        if imgui.Checkbox("Не прерывать открытый диалог",
                                        ...
                                        if cfg.refreshPostponeOnDialog then
                                            ...
                                            if imgui.SliderInt("##refreshPostpone", imcfg.refreshPostponeMinutes, 1, 5,
                                                    (string.format("отложить на %d мин.", imcfg.refreshPostponeMinutes[0]))) then
                            ...
                            elseif data.cheatSubTab == 1 then
                                if not cfg.cheatModeEnabled then
                                    ...
                                    if imgui.Checkbox("Включить автооплату налогов", imcfg.autoPayTaxesEnabled) then
                                    ...
                                    if cfg.autoPayTaxesEnabled then
                                        ...
                                        if imgui.Checkbox("Вместе с автосбором", imcfg.autoPayTaxesWithCollect) then
                                        ...
                                        if imgui.Checkbox("По таймеру", imcfg.autoPayTaxesByTimer) then
                                        ...
                                        if cfg.autoPayTaxesByTimer then
                                            ...
                                            if imgui.SliderInt("##taxInt", imcfg.autoPayTaxesInterval, 1, 48,
                                                    (string.format("каждые %d ч.", imcfg.autoPayTaxesInterval[0]))) then
                                    ...
                                    if imgui.Checkbox("Включить автопополнение", imcfg.autoTopUpEnabled) then
                                    ...
                                    if cfg.autoTopUpEnabled then
                                        ...
                                        if imgui.Checkbox("Вместе с автосбором", imcfg.autoTopUpWithCollect) then
                                        ...
                                        if imgui.Checkbox("При низком балансе", imcfg.autoTopUpByThreshold) then
                                        ...
                                        if cfg.autoTopUpByThreshold then
                                            ...
                                            if imgui.SliderInt("##topUpThr", imcfg.autoTopUpThreshold, 500000, 20000000,
                                                    ("$" .. utils.formatNumber(imcfg.autoTopUpThreshold[0]))) then
                                        ...
                                        if imgui.Checkbox("По таймеру", imcfg.autoTopUpByTimer) then
                                        ...
                                        if cfg.autoTopUpByTimer then
                                            ...
                                            if imgui.SliderInt("##topUpInt", imcfg.autoTopUpTimerInterval, 1, 48,
                                                    (string.format("каждые %d ч.", imcfg.autoTopUpTimerInterval[0]))) then
                            ...
                            elseif data.cheatSubTab == 2 then
                                ...
                                if blockReminder then
                                    ...
                                    imgui.Checkbox("Напоминание о BTC+ASC", dummy)
                                    imgui.PopStyleColor(4)
                                    imgui.Hint(
                                        "Недоступно пока включён автосбор\nили умный автосбор.")
                                else
                                    if imgui.Checkbox("Напоминание о BTC+ASC", imcfg.reminderEnabled) then
                                ...
                                if cfg.reminderEnabled and not blockReminder then
                                    ...
                                    if imgui.SliderInt("##btcThr", imcfg.btcThreshold, 10, 2000,
                                            (string.format("порог: %d BTC+ASC", imcfg.btcThreshold[0]))) then
                                        cfg.btcThreshold = imcfg.btcThreshold[0]; save()
                                    end
                                    if imgui.SliderInt("##remInt", imcfg.reminderInterval, 1, 60,
                                            (string.format("каждые %d мин.", imcfg.reminderInterval[0]))) then
                                ...
                                if cfg.cheatModeEnabled then
                                    ...
                                    if imgui.Checkbox("Уведомления автосбора", imcfg.notifyAutoCollectEnabled) then
                                    ...
                                    if cfg.autoCollectEnabled then
                                        ...
                                        if imgui.SliderInt("##nBefore", imcfg.notifyBeforeSec, 30, 600,
                                                (string.format("за %d сек.", imcfg.notifyBeforeSec[0]))) then
                                    ...
                                    if imgui.Selectable("Предпросмотр окна", false) then
                                        data.notifyWindow.btcAmount  = 150
                                        data.notifyWindow.mode       = 'reminder'
                                        data.notifyWindow.autoHideAt = os.time() + cfg.notifyShowDuration
                                        data.notifyWindow.isPreview  = true
                ...
                elseif data.settingsTab == 3 then
                    ...
                    if imgui.Checkbox("Приостанавливать действия на PayDay", imcfg.pauseOnPayday) then
                    ...
                    if imgui.Checkbox("Ждать подключения к серверу", imcfg.waitForConnection) then
                    ...
                    if cfg.waitForConnection then
                        ...
                        if imgui.SliderInt("##delayAfterConnect", imcfg.delayAfterConnectMin, 5, 20,
                                (string.format("задержка %d мин. после подключения",
                    ...
                    imgui.PushItemWidth(-1)
                    if imgui.SliderInt("##pause", imcfg.pause_duration, 150, 300, "%d мс") then
                        cfg.pause_duration = imcfg.pause_duration[0]; save()
                    end
                    ...
                    imgui.Text("Акцентный цвет интерфейса")
                    imgui.SameLine()
                    ...
                    for presetIdx, preset in ipairs(accentPresets) do
                        ...
                        if imgui.Button(("##accentPreset" .. presetIdx), imgui.ImVec2(28, 28)) then
                ...
                elseif data.settingsTab == 4 then
                    ...
                    imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.20, 0.30, 0.48, 1))
                    imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.22, 0.32, 0.50, 1))
                    if imgui.Button(fa.ARROW_UP_FROM_BRACKET .. "  " .. ("Открыть в отдельном окне"),
...
imgui.OnFrame(
    ...
    function(self)
        ...
        if imgui.Begin("##updateWin", updateState.showPopup,
            ...
            local titleText = "Доступно обновление"
            ...
            if imgui.Button(fa.DOWNLOAD .. "  Обновить сейчас", imgui.ImVec2(halfW, 36)) then
                downloadAndUpdate()
            end
            imgui.PopStyleColor(3)
            ...
            if imgui.Button(fa.CLOCK .. "  Напомнить позже", imgui.ImVec2(halfW, 36)) then
                updateState.showPopup[0] = false
                updateState.postponeUntil = os.time() + 30 * 60
                updateState.flashOpenAsked = false
                updateState.declined     = false
...
imgui.OnFrame(
    ...
    function(self)
        ...
        local function renderLogEntry(entry, childPrefix, h)
            ...
            if eDetail ~= "" then
                local dw = imgui.CalcTextSize((eDetail)).x
        ...
        local function renderEmptyLogs()
            ...
            for _, l in ipairs(lines) do
                imgui.SetCursorPosX(cW / 2 - imgui.CalcTextSize((l[2])).x / 2)
        ...
        if imgui.Begin("##logsWin", data.showLogsWindow,
            ...
            if imgui.Button("Общее", imgui.ImVec2(tabW, 28)) then _logsActiveTab = 0 end
            ...
            if imgui.Button("По дням", imgui.ImVec2(tabW, 28)) then _logsActiveTab = 1 end
            ...
            if _logsActiveTab == 0 then
                ...
                if imgui.BeginChild("##logsRightCol", imgui.ImVec2(colW, 0), true,
                        imgui.WindowFlags.NoScrollWithMouse) then
                    local periodLabels = { "Всё время", "Сегодня", "Неделя", "Месяц" }
                    imgui.PushItemWidth(-1)
...
imgui.OnFrame(function()
    ...
    if imgui.Begin("Mining Tools##MainWin", data.showHouseControlWindow, mainWindowFlags) then
        ...
        local function DrawStatTile(childId, icon, label, value, valColor, hintText)
            ...
            local labelSize = imgui.CalcTextSize((labelParsed))
            local valueParsed = value:gsub("{.-}", "")
            local valueSize = imgui.CalcTextSize((valueParsed))
        ...
        if logTotalSessions == 0 then
            summaryStr = "Нет записей"
        else
            summaryStr = (string.format("Собрано за всё время: %d BTC", logTotalBtc))
            if logTotalAsc > 0 then
                summaryStr = summaryStr .. (string.format("  /  %d ASC", logTotalAsc))
            end
            summaryStr = summaryStr .. (string.format("   ·   %d записей", logTotalSessions))
        end
        ...
        if isLogsHovered then
            imgui.SetTooltip((data.showLogsWindow[0] and "Закрыть историю" or "Открыть историю"))
        end
                ...
                for i, house in ipairs(filteredHouses) do
                    ...
                    if not isKnownNoBasement then
                        if status and status.lastCheck > 0 and status.minCoolant <= 100 then
                            ...
                            local barLabel = (string.format("%.1f%%", status.minCoolant))
                            imgui.PushStyleColor(imgui.Col.PlotHistogram, barColor)
                            ...
                            imgui.PushStyleColor(imgui.Col.FrameBg, imgui.ImVec4(0.2, 0.2, 0.2, 1.0))
                            imgui.ProgressBar(0, imgui.ImVec2(barW, 15), "Нет данных")
                            imgui.PopStyleColor()
                        end
                    else
                        imgui.PushStyleColor(imgui.Col.FrameBg, imgui.ImVec4(0.2, 0.2, 0.2, 1.0))
                        imgui.ProgressBar(0, imgui.ImVec2(barW, 15), "Нет подвала")
                        imgui.PopStyleColor()
                    end
                    ...
                    if imgui.BeginPopup("house_context_menu_" .. house.house_number) then
                        ...
                        if imgui.MenuItemBool((excluded and "Снять метку 'Пропускать'" or "Пропускать дом"), nil, excluded) then
                            cfg.excludedHouses[houseStr] = not excluded and true or nil
                            save()
                        end

                        if imgui.MenuItemBool("Найти дом (/findihouse)") then
                            sampSendChat(string.format("/findihouse %d", house.house_number))
                        end

                        if not data.working then
                            imgui.Separator()
                            if imgui.MenuItemBool("Обновить статус этого дома") then
                                lua_thread.create(function()
                                    ...
                                    imgui.addNotification((string.format("Дом №%d обновлён", house.house_number)))
                                end)
                            end
                            if not houseFilter.isExcluded(house.house_number) and not houseFilter.hasNoBasement(house.house_number) then
                                if imgui.MenuItemBool("Зайти в дом") then
...
function __i__progressPanel()
    ...
    local counterText = (string.format("Дом: %d / %d", data.progressCurrent,
        data.progressTotal > 0 and data.progressTotal or 0))
    ...
    if data.progressHouseTotal > 0 then
        local houseText = (string.format("Карта: %d / %d", data.progressHouseCurrent, data.progressHouseTotal))
    ...
    if data.isWaitingPayday then
        ...
        if imgui.Button(fa.STOP .. " Остановить", imgui.ImVec2(95, 40)) then
            data.stopAction = true
        ...
        if imgui.Button(fa.FORWARD_STEP .. " Пропустить", imgui.ImVec2(95, 40)) then
            data.skipPayday = true
            data.paydaySkippedAt = os.time()
        end
        imgui.PopStyleColor(3)
        ...
        imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(1.0, 0.3, 0.3, 1.0))
        imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.6, 0.1, 0.1, 1.0))
        if imgui.Button(fa.STOP .. " Остановить", imgui.ImVec2(200, 40)) then
            data.stopAction = true
        end
        imgui.PopStyleColor(3)
    end
...
function renderResetConfirm(idSuffix, timerStartedAt, title, subtitle, onConfirm, onCancel)
    ...
    if imgui.Begin("##" .. idSuffix, nil,
        ...
        if canConfirm then
            ...
            if imgui.Button("Сбросить", imgui.ImVec2(halfW, 28)) then onConfirm() end
            imgui.PopStyleColor(3)
        else
            ...
            imgui.PushStyleColor(imgui.Col.Text, imgui.ImVec4(0.5, 0.5, 0.5, 1))
            imgui.Button((string.format("Сбросить (%dс)", remaining)), imgui.ImVec2(halfW, 28))
            imgui.PopStyleColor(4)
        end
        imgui.SameLine()
        if imgui.Button("Отмена", imgui.ImVec2(halfW, 28)) then onCancel() end
